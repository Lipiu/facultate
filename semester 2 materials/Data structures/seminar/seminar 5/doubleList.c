...
DoubleList insertMiddle(DoubleList list, BankCard bc) {
    int count = 0;
...
int main()
{
	...
	printf("\nDouble list after middle insertion:\n");
	BankCard newBc;
	newBc.balance = 0;
	strcpy(newBc.card_no, "2022222");
	strcpy(newBc.holder, "Middle holder");
	newBc.currency = "RON";
	strcpy(newBc.exp_date, "2");
	insertMiddle(d_list, newBc);
	parse_list(d_list);
