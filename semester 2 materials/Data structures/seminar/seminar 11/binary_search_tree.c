...
{
	BankCard data;
    struct BSTNode* left; // mem address of the next item (node) in the chain
...
BSTNode* insert_node_BST(BSTNode* node, BankCard data, unsigned char* success_flag){
    if(node != NULL){
        ...
        if(strcmp(data.card_no, node->data.card_no) < 0){
            node->left = insert_node_BST(node->left, data, success_flag); // continue searching on left subtree
        }
        else{
            //go right
            if(strcmp(data.card_no, node->data.card_no) > 0){
                node->right = insert_node_BST(node->right, data, success_flag); // continue searching on right subtree
            }
            // duplicate
            else{
                // there is a duplicated key in BST -> drop off insertion
                *success_flag = 0;
            }
        }
        ...
        // we get to the NULL position where the new node must be added to the tree structure
...
int main()
{
	...
	while (fgets(buffer, sizeof(buffer), f))
	{
        ...
        unsigned char inserted_flag; // not mandatory to initialize here
        root = insert_node_BST(root, card, &inserted_flag);

        if(inserted_flag != 0){
            printf("Card no# %s has been successfully inserted into BST.\n", card.card_no);
        }
        else{
            printf("Card no# %s has not been inserted into BST.\n", card.card_no);
