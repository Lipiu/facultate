...
def s_turneu(pop,k,nr):
    ...
    # I: pop - populatia curenta
    #    k - dimensiune turneu
    #    nr - numarul de indivizi selectati (m pentru parinti, m/2 pentru generatia urmatoare)
    ...
    for i in range(nr):
        ...
        valori=[pop[turneu[i]][m-1] for i in range(k)]
...
def s_genitor(pop,desc,nr):
    ...
    dim,_=numpy.shape(pop)
    b1=pop[pop[:,-1].argsort()]
    b2=desc[-desc[:,-1].argsort()]
    noua=b1.copy()
    noua[0:nr,:]=b2[0:nr,:]
    # amestecare populatie
    amestec=numpy.random.permutation(dim)
    rez=noua[amestec]
    return rez
